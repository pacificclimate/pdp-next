...
export const timeSliderContainer = document.getElementById(
  "timeSliderContainer",
);
...
export function startStatusSpinner(message) {
  ...
  statusSpinnerTimer = setInterval(() => {
    statusSpinnerFrame =
      (statusSpinnerFrame + 1) % STATUS_SPINNER_FRAMES.length;
    const elapsedSeconds = Math.floor(
      (Date.now() - statusSpinnerStartedAt) / 1000,
    );
    setStatus(
      `${STATUS_SPINNER_FRAMES[statusSpinnerFrame]} ${message} (${elapsedSeconds}s)`,
    );
  }, 160);
}
